 ...
 * A filterset is a list of whitelist/blacklist rules defined by a given path in the plugin config and
 * requiring a given permission to be applicable.
 *
 * @author crashdemons
 */
public class FilterSet{
	...
	public FilterSet(JavaPlugin pl){
		...
		filters.put("group-whitelists", new Filter(FilterResult.ALLOW_FINAL, FilterResult.SKIP, args -> {
		...
		filters.put("group-blacklists", new Filter(FilterResult.DENY_FINAL, FilterResult.SKIP, args -> {
			for(FilterSet groupFilterSet : filterGroups.values()){
				if(groupFilterSet.hasPermission(args)){
		...
		filterAll = new Filter(FilterResult.ALLOW_FINAL, FilterResult.DENY_FINAL, args -> {
	...
	private void loadDefaultResult(ConfigurationSection config){
		...
		if(resultName == null){
			// should not occur because we have a defined default config value - but this corrects a warning.
		...
		try{
			...
			plugin.getLogger().info("Loaded default action: " + defaultResult.name());
		}catch(Exception e){
	...
	private void loadLists(ConfigurationSection config, boolean logoutput){
		...
		try{
			...
			if(logoutput){
				plugin.getLogger().warning("error loading visible-commands, skipping.");
			}
		}
		if(logoutput){
			plugin.getLogger().fine("Loaded " + visibleCommands.size() + " whitelist entries.");
		}
		...
		try{
			...
			if(logoutput){
				plugin.getLogger().warning("error loading invisible-commands, skipping.");
			}
		}
		if(logoutput){
			plugin.getLogger().fine("Loaded " + invisibleCommands.size() + " blacklist entries.");
		}
	}
	...
	private List<String> loadFilterOrder(ConfigurationSection config, boolean logoutput){
		...
		if(config.contains("filter-order", true)){
			...
			if(logoutput){
				plugin.getLogger().fine("Loaded filter-order: " + order.toString());
			}
			return order;
		}
		else{
			...
			if(defaults == null){
				// should not occur because we define a default configuration file internally, but this fixes a warning
	...
	// loads the set of filters for each group as a new FilterSet with the
	// appropriate group permission and stores it in filterGroups
	// Note: Group FilterSets have their internal filters accessed directly by
	// the parent FilterSet, and do not have to be Built or Enabled by a Filter-Order.
	private void loadGroups(ConfigurationSection config, boolean logoutput, List<String> filterOrder){
		...
		plugin.getLogger().fine("Loaded " + filterGroups.size() + " groups.");
	}
	...
	public void load(ConfigurationSection config){
		// detect group load and don't load nested groups etc.
		...
		plugin.getLogger().fine("Loaded " + filtersEnabled.size() + " filters.");
	}
}
